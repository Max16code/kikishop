...
export default async function activewearsPage() {
  await connectToDatabase();

  // Consistently use lowercase 'jeans' for category matching
  const activewears = await Product.find({ category: 'activewears' }).lean();

  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">activewears</h1>

      {jeans.length === 0 ? (
        <p className="text-gray-500">No products found in this category.</p>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {jactivewears.map((item) => (
